#!/bin/bash
# start.sh - Trampala Docker Environment

set -e

# Color codes for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Function to print colored output
print_status() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

print_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

print_error() {
    echo -e "${RED}[ERROR]${NC} $1"
}

# Function to create .env from .env.example
create_env_from_example() {
    local dir=$1
    local name=$2
    
    if [ ! -f "$dir/.env" ]; then
        if [ -f "$dir/.env.example" ]; then
            print_warning "$name .env file not found, creating from .env.example..."
            cp "$dir/.env.example" "$dir/.env"
            
            # Generate APP_KEY for Laravel if it's empty
            if [ "$name" = "Laravel" ] && [ -f "$dir/.env" ]; then
                if grep -q "^APP_KEY=$" "$dir/.env"; then
                    print_status "Generating Laravel APP_KEY..."
                    # Generate a base64 encoded random key
                    KEY=$(openssl rand -base64 32)
                    # Use sed compatible with macOS
                    sed -i '' "s/^APP_KEY=$/APP_KEY=base64:$KEY/" "$dir/.env"
                    print_success "APP_KEY generated successfully"
                fi
            fi
            
            print_success "$name .env file created from .env.example"
        else
            print_error "$name .env.example file not found in $dir!"
            print_error "Please create $dir/.env.example first"
            exit 1
        fi
    else
        print_status "$name .env file already exists"
    fi
}

# Check and create .env files
create_env_from_example "laravel" "Laravel"
create_env_from_example "nuxt" "Nuxt.js"

# Load environment variables from laravel directory
if [ -f laravel/.env ]; then
    source laravel/.env
else
    print_error "Failed to create or load laravel/.env"
    exit 1
fi

# Basic environment check
if [ -z "$HOST_UID" ]; then
    print_warning "HOST_UID not set, using default 501"
fi

# Set hot reload environment variables for Docker development
export WATCHPACK_POLLING=true
export CHOKIDAR_USEPOLLING=true
export CHOKIDAR_INTERVAL=1000
print_status "Hot reload environment variables set for Docker development"

print_status "Starting Trampala development environment..."

# Function to check and create Docker network
create_network() {
    local network_name="trampala_network"
    
    print_status "Checking Docker network: $network_name"
    
    if docker network inspect $network_name >/dev/null 2>&1; then
        print_status "Network $network_name already exists"
    else
        print_status "Creating Docker network: $network_name"
        if docker network create $network_name >/dev/null 2>&1; then
            print_success "Network $network_name created successfully"
        else
            print_error "Failed to create network $network_name"
            exit 1
        fi
    fi
}

# Create network before starting services
create_network

# Check if base image exists before building
check_base_image() {
    if docker image inspect trampala-base:latest >/dev/null 2>&1; then
        print_status "Base image already exists, skipping build"
        return 0
    else
        print_status "Building base image..."
        if docker build -t trampala-base:latest -f docker/base.Dockerfile .; then
            print_success "Base image built successfully"
            return 0
        else
            print_error "Failed to build base image"
            exit 1
        fi
    fi
}

# Check and build base image only if needed
check_base_image


# Build and start services (without forcing rebuild)
print_status "Starting Docker containers..."
docker compose up -d

# Check if services are running
print_status "Checking service status..."
sleep 10

# Check Laravel container status with better error handling
if docker ps | grep -q trampala_laravel; then
    print_success "Trampala API container is running!"
    print_status "API available at: http://localhost:8080"
else
    print_warning "Laravel container may be starting, waiting..."
    sleep 10
    if ! docker ps | grep -q trampala_laravel; then
        print_error "Trampala API failed to start"
        print_status "Showing last 30 lines of Laravel logs:"
        docker compose logs --tail=30 laravel
    fi
fi

if docker ps | grep -q trampala_postgres; then
    print_success "PostgreSQL is running!"
    
    # Check if Laravel container is actually running before executing commands
    if ! docker ps | grep -q trampala_laravel; then
        print_warning "Laravel container is not running, skipping configuration steps"
    else
        # Fix database host in .env file if needed
        print_status "Checking database configuration..."
        docker exec trampala_laravel sed -i 's/DB_HOST=postgres/DB_HOST=trampala_postgres/g' /var/www/html/api/.env 2>/dev/null || true
        docker exec trampala_laravel sed -i 's/REDIS_HOST=redis/REDIS_HOST=trampala_redis/g' /var/www/html/api/.env 2>/dev/null || true
        docker exec trampala_laravel sed -i 's/MAIL_HOST=mailhog/MAIL_HOST=trampala_mailhog/g' /var/www/html/api/.env 2>/dev/null || true
        print_success "Database configuration updated"
    fi
    
    # Wait additional time for PostgreSQL to be fully ready
    print_status "Waiting for PostgreSQL to be fully ready..."
    sleep 10
    
    # Only run Laravel setup if container is running
    if docker ps | grep -q trampala_laravel; then
        # Clear composer cache first
        print_status "Clearing Composer cache..."
        docker exec trampala_laravel composer clear-cache 2>/dev/null || true
        
        # Install composer dependencies first
        print_status "Installing Composer dependencies..."
        if docker exec trampala_laravel composer install 2>/dev/null; then
            print_success "Composer dependencies installed successfully!"
        else
            print_warning "Composer dependencies may already be installed"
        fi
        
        # Clear Laravel caches
        print_status "Clearing Laravel caches..."
        docker exec trampala_laravel php artisan config:clear 2>/dev/null || true
        docker exec trampala_laravel php artisan cache:clear 2>/dev/null || true
        
        # Test database connection first
        print_status "Testing database connection..."
        if docker exec trampala_laravel php artisan tinker --execute="DB::connection()->getPdo(); echo 'Database connected successfully!';" >/dev/null 2>&1; then
            print_success "Database connection test passed!"
        else
            print_warning "Database connection test failed, will retry after fixing configuration"
            print_status "Checking network connectivity..."
            docker exec trampala_laravel ping -c 2 trampala_postgres 2>/dev/null || true
        fi
        
        # Create storage link for file access
        print_status "Creating storage symlink..."
        docker exec trampala_laravel php artisan storage:link 2>/dev/null || true
        print_success "Storage symlink created or already exists!"
        
        # Run migrations and seeders
        print_status "Running Laravel migrations and seeders..."
        if docker exec trampala_laravel php artisan migrate:fresh --seed 2>/dev/null; then
            print_success "Database migrations and seeders completed successfully!"
        else
            print_warning "Migrations may have failed or database not ready yet"
        fi
    else
        print_warning "Laravel container not running, skipping Laravel setup"
    fi
else
    print_error "PostgreSQL failed to start"
    docker compose logs postgres
    exit 1
fi

if docker ps | grep -q trampala_redis; then
    print_success "Redis is running!"
else
    print_error "Redis failed to start"
    docker compose logs redis
fi

if docker ps | grep -q trampala_mailhog; then
    print_success "MailHog is running!"
    print_status "MailHog Web UI available at: http://localhost:8025"
else
    print_warning "MailHog may not be running properly"
fi

if docker ps | grep -q trampala_nuxt; then
    print_success "Nuxt.js Frontend container is running!"
    
    # Install npm dependencies
    print_status "Installing Nuxt.js dependencies..."
    if docker exec trampala_nuxt npm install; then
        print_success "Nuxt.js dependencies installed successfully!"
    else
        print_error "Failed to install Nuxt.js dependencies"
        exit 1
    fi
    
    # Wait for Nuxt.js to be ready
    print_status "Waiting for Nuxt.js to start (this may take 30-60 seconds)..."
    count=0
    while [ $count -lt 12 ]; do
        if curl -f http://localhost:3000 >/dev/null 2>&1; then
            print_success "Nuxt.js Frontend is running and responsive!"
            print_status "Frontend available at: http://localhost:3000"
            break
        else
            print_status "Waiting for Nuxt.js to be ready... ($((count+1))/12)"
            sleep 5
            count=$((count+1))
        fi
    done
    
    if [ $count -eq 12 ]; then
        print_error "Nuxt.js Frontend failed to start properly"
        print_status "Checking Nuxt.js logs..."
        docker compose logs --tail=50 nuxt
        exit 1
    fi
else
    print_error "Nuxt.js Frontend container failed to start"
    docker compose logs nuxt
    exit 1
fi

print_success "All services started successfully!"
print_status ""
print_status "🚀 Development environment is ready!"
print_status "📡 API: http://localhost:8080"
print_status "🖥️  Frontend: http://localhost:3000"
print_status "📧 MailHog: http://localhost:8025"
print_status "🗄️  PostgreSQL: localhost:5432"
print_status ""
print_status "To view logs: docker compose logs -f"
print_status "To stop: docker compose down"